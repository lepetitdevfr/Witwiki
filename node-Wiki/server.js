...
app.get('/getMessageOut', function (req, res) {
	...
	var params = req.query;
	connection.query('SELECT m.id, m.title, m.content, m.date, m.lu, u.pseudo FROM message AS m, user AS u WHERE m.id_to = u.id AND id_from='+params.idUser, function (error, results, fields) {
		if (error) {
			res.json(error)
		}else{
			res.json(results);
